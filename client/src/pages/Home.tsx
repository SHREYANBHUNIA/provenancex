  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        {activeNav === "Lineage explorer" && <>
          <section className="destination-hero"><div><p className="section-kicker">Interactive graph workspace</p><h2>Explore the complete dependency path.</h2><p>Choose a graph direction, inspect an asset, and move through the transformation chain without leaving the workspace.</p></div><div className="destination-stat"><span>VISIBLE ASSETS</span><strong>{traceGraph.nodes.length}</strong></div></section>
          <section className="panel explorer-panel">
            <div className="panel-header"><div><p className="section-kicker">Graph traversal</p><h3>{traceMode} dependencies for {selected.name}</h3></div><div className="graph-actions"><div className="segmented-control">{(["upstream", "downstream", "all"] as const).map(mode => <button key={mode} onClick={() => setTraceMode(mode)} className={traceMode === mode ? "selected" : ""}>{mode}</button>)}</div><Button variant="outline" className="report-button" onClick={() => setShowReport(true)}><FileText size={15} />Provenance report</Button></div></div>
            <div className="lineage-body"><div className="graph-stage"><LineageGraph nodes={traceGraph.nodes} edges={traceGraph.edges} selectedId={selectedId} onSelect={onSelect} /></div><aside className="asset-drawer"><div className="drawer-top"><span>SELECTED ASSET</span><StatusPill status={selected.status} /></div><div className="asset-icon"><SelectedIcon size={23} /></div><h4>{selected.name}</h4><p className="asset-version">{selected.version}</p><p className="asset-description">{selected.description}</p><div className="metadata-list"><div><span>TYPE</span><strong>{selected.kind}</strong></div><div><span>OWNER</span><strong>{selected.owner}</strong></div><div><span>UPDATED</span><strong>{selected.updatedAt}</strong></div>{Object.entries(selected.metadata).slice(0, 2).map(([key, value]) => <div key={key}><span>{key.toUpperCase()}</span><strong className="mono">{value}</strong></div>)}</div><div className="drawer-actions"><Button onClick={() => setShowReport(true)}><FileText size={15} />View report</Button><Button variant="outline" onClick={() => setReplayed(true)}><RefreshCw size={15} />Replay</Button></div></aside></div>
            <div className="graph-footer"><div className="legend"><span><i className="legend-dot dataset" />Dataset</span><span><i className="legend-dot transform" />Transform</span><span><i className="legend-dot feature" />Features</span><span><i className="legend-dot model" />Model</span><span><i className="legend-dot result" />Result</span></div><p>Click any node to make it the trace subject</p></div>
          </section>
        </>}

        {activeNav === "Runs" && <>
          <section className="destination-hero"><div><p className="section-kicker">Execution ledger</p><h2>Recorded runs and replay state.</h2><p>Every run stores its versioned inputs, environment, output contract, duration, and reproducibility status.</p></div><div className="destination-stat"><span>REPLAYABLE</span><strong>91</strong></div></section>
          <section className="panel destination-table"><div className="panel-header"><div><p className="section-kicker">Pipeline run history</p><h3>weekly_risk_scoring</h3></div><Button onClick={() => setReplayed(true)}><Play size={15} fill="currentColor" />Replay latest</Button></div><div className="run-ledger-head"><span>RUN</span><span>INPUTS & OUTPUT</span><span>EXECUTION</span><span>STATUS</span></div>{data.runHistory.map(run => <div className="run-ledger-row" key={run.id}><div><strong>{run.label}</strong><small className="mono">{run.id}</small></div><div><p>orders_raw + customers_raw</p><small>→ q1_retention_risk · 4.1M predictions</small></div><div><p>{run.time}</p><small>{run.duration}</small></div><div><StatusPill status={run.status as ProvenanceStatus} /><Button variant="outline" size="sm" onClick={() => { setSelectedId("result-risk"); setActiveNav("Lineage explorer"); }}>Trace</Button></div></div>)}</section>
        </>}

        {activeNav === "Versions" && <>
          <section className="destination-hero"><div><p className="section-kicker">Definition diffing</p><h2>Compare every pipeline revision.</h2><p>Review input contracts, transformed stages, and freshness policy changes before you replay or promote a version.</p></div><div className="destination-stat"><span>CURRENT VERSION</span><strong>v2.8.0</strong></div></section>
          <section className="versions-grid">{data.pipelineVersions.map(version => <article className={`panel version-card ${version.status === "current" ? "current" : ""}`} key={version.version}><div className="version-card-top"><span className="section-kicker">{version.status === "current" ? "Current" : "Baseline"}</span><BadgeStatus label={version.status === "current" ? "Active" : "Archived"} /></div><h3>{version.version}</h3><p className="version-date">{version.createdAt} · {version.author}</p><div className="version-stats"><span>{version.inputs} inputs</span><span>{version.transforms} stages</span><span>{version.output}</span></div><div className="version-changes">{version.changes.map(change => <p key={change}><Check size={13} />{change}</p>)}</div></article>)}</section>
          <section className="panel compare-inline"><div><p className="section-kicker">Selected comparison</p><h3>v2.7.4 → v2.8.0</h3><p>Three material changes alter the reproducibility profile of this pipeline.</p></div><Button onClick={() => setShowCompare(true)}>Open comparison <GitCompareArrows size={16} /></Button></section>
        </>}

        {activeNav === "Reports" && <>
          <section className="destination-hero"><div><p className="section-kicker">Evidence library</p><h2>Exportable provenance reports.</h2><p>Generate a concise evidence record for a dataset, transformation, model, or published result.</p></div><div className="destination-stat"><span>AVAILABLE REPORTS</span><strong>{data.graph.nodes.length}</strong></div></section>
          <section className="reports-grid">{data.graph.nodes.map(node => { const NodeIcon = kindIcon[node.kind]; return <article className="panel report-card" key={node.id}><div className="report-card-icon"><NodeIcon size={18} /></div><div className="report-card-title"><div><p className="section-kicker">{node.kind}</p><h3>{node.name}</h3><span className="mono">{node.version}</span></div><StatusPill status={node.status} /></div><p>{node.description}</p><div className="report-card-footer"><span>{node.owner}</span><Button variant="outline" size="sm" onClick={() => { setSelectedId(node.id); setShowReport(true); }}>Open report <FileText size={14} /></Button></div></article>; })}</section>
        </>}
      </main>
